...
// get the max number of cbs what can attachable to one cvm.
func (qcloud *QCloud) GetMaxAttachLimit() (uint64, error) {
	instanceId, err := qcloud.metaData.InstanceID()
	if err != nil {
		klog.Errorf("Get localhost instance-id failed: %v\n.", err)
		return 0, err
	}

	req := cbsv3.NewDescribeInstancesDiskNumRequest()
	req.InstanceIds = append(req.InstanceIds, &instanceId)
	resp, err := qcloud.cbsV3.DescribeInstancesDiskNum(req)
	if err != nil {
		klog.Errorf("DescribeInstancesDiskNum failed: %v\n", err)
		return 0, err
	}

	if len(resp.Response.AttachDetail) <= 0 {
		return 0, nil
	}

	return *resp.Response.AttachDetail[0].MaxAttachCount, nil
}
